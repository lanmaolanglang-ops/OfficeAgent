...
class FileManager:
    ...
    def _save_metadata(self):
        ...
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump([v.to_dict() for v in self.files.values()], f, ensure_ascii=False)
    ...
    def save_upload(self, filename: str, content: bytes,
        ...
        if len(content) > settings.max_file_size:
            raise FileError(f"文件大小超过限制 {settings.max_file_size // (1024*1024)}MB")
        ...
        file_type = self.TYPE_MAP.get(ext, "unknown")
        info = FileInfo(
            file_id=file_id,
    ...
    def save_output(self, source_path: str, original_name: str = None,
                    file_id: str = None) -> FileInfo:
        """保存输出文件"""
        if not os.path.exists(source_path):
            raise FileError(f"源文件不存在: {source_path}")

        ext = os.path.splitext(source_path)[1].lower()
        file_id = file_id or f"out_{uuid.uuid4().hex[:12]}"
        original_name = original_name or os.path.basename(source_path)
        stored_name = f"{file_id}{ext}"
        stored_path = os.path.join(self.output_dir, stored_name)
    ...
    def get_file_path(self, file_id: str) -> str:
        """获取文件路径"""
        return self.get_file(file_id).stored_path
